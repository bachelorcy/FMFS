...
class FMFS(nn.Module):
    ...
    def forward(self, x):
        ...
        x4_f_a = torch.add(self.conv_4(torch.cat((x4_up_2, f4_en_m),1)), output_ncd)   # (8, 1, 28, 28)
        ...
        x3_f_a = torch.add(self.conv_3(torch.cat((x3_up, f3_en_m),1)), x4_f_a)      # (8, 1, 28, 28)
        ...
        x2_f_a = torch.add(self.conv_2(torch.cat((x2, f2_en_m), 1)), x3_f_a)     # (8, 1, 28, 28)
        x2_f_a_en1 = F.interpolate(x2_f_a, size=(56,56), mode='bilinear',align_corners=True)    # (8, 1, 56, 56)
        x2_f_a_en2 = x2_f_a_en1.expand(-1, 64, -1, -1)                                          # (8, 1, 56, 56)


        # 1st stage
        x1 = torch.cat((s1, x2_up), 1)
        x1 = self.frb_1(x1)                                                                      # (8, 64, 56, 56)
        # x1_f = self.fem_1(torch.cat((x1, x2_f_a_en2, x3_f_a_en2, x4_f_a_en3 ), 1))   # (8, 64, 56, 56)

        f1 = compute_attention(x2_f_a) + f2                                                      # (8, 1, 28, 28)
        f1_en = F.interpolate(f1, size=(56,56),
                                   mode='bilinear',align_corners=True).expand(-1, 64, -1, -1)     # (8, 64, 56, 56)
        f1_en_m = f1_en.mul(x1)
        x1_f_a = torch.add(self.conv_1(torch.cat((x1, f1_en_m), 1)), x2_f_a_en1)       # (8, 1, 56, 56)
...
# # Calculate FLOPs and MACs(PVTv2)   FLOPs: 20.32 G   MACs: 20.29 G   parameters: 114.81 M
