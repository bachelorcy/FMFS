...
from FMFS.lib.module import FeatureRefineBlock, NeighborConnectionDecoder, Octave, FrequencyEnhancedModule, compute_attention
from FMFS.lib.PVTv2 import pvt_v2_b4
...
# Abolation of octave
class Octave_Abolation_ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Octave_Abolation_ConvBlock, self).__init__()
...
class FMFS(nn.Module):
    ...
    def forward(self, x):
        ...
        o1 = self.octave_1(s2)                                                                  # (8, 128, 28, 28)
        o2 = self.octave_2(s3)                                                                  # (8, 128, 28, 28)
        o3 = self.octave_3(s4)                                                                  # (8, 128, 28, 28)
        ...
        s4_en = F.pixel_shuffle(s4, 4)
        x4_f = self.fem_4(torch.cat((s4_en, output_en), 1))                      # (8, 48, 28, 28)

        f4 = compute_attention(output_ncd)                                                      # (8, 1, 28, 28)
        f4_en = f4.expand(-1, 32, -1, -1)                                                       # (8, 32, 28, 28)
        f4_en_m = f4_en.mul(s4_en)                                                            # (8, 32, 28, 28)
        ...
        x4_f_a_en3 = F.interpolate(x4_f_a, size=(56,56),
                                   mode='bilinear',align_corners=True).expand(-1, 64, -1, -1)   # (8, 96, 56, 56)
        ...
        s3_en = F.pixel_shuffle(s3, 2)
        x3_f =self.fem_3(torch.cat((s3_en, x4_f_a_en1), 1))                        # (8, 96, 28, 28)

        f3 = compute_attention(x4_f_a)                                                          # (8, 1, 28, 28)
        f3_en = f3.expand(-1, 80, -1, -1)                                                       # (8, 96, 28, 28)
        f3_en_m = f3_en.mul(s3_en)                                                              # (8, 96, 28, 28)
        x3_f_a = torch.add(self.conv_3(torch.cat((x3_f, f3_en_m),1)), x4_f_a)      # (8, 1, 28, 28)
        x3_f_a_en1 = x3_f_a.expand(-1, 128, -1, -1)                                            # (8, 192, 28, 28)
        ...
        x2_f = self.fem_2(torch.cat((s2, x3_f_a_en1, x4_f_a_en2 ), 1))             # (8, 192, 28, 28)

        f2 = compute_attention(x3_f_a) + f3                                                     # (8, 1, 28, 28)
        f2_en = f2.expand(-1, 128, -1, -1)                                                      # (8, 128, 28, 28)
        f2_en_m = f2_en.mul(s2)                                                                 # (8, 128, 28, 28)
        ...
        x1_f = self.fem_1(torch.cat((s1, x2_f_a_en2, x3_f_a_en2, x4_f_a_en3 ), 1))   # (8, 96, 56, 56)

        f1 = compute_attention(x2_f_a) + f2                                                      # (8, 1, 56, 56)
        f1_en = F.interpolate(f1, size=(56,56),
                                   mode='bilinear',align_corners=True).expand(-1, 64, -1, -1)     # (8, 96, 56, 56)
        f1_en_m = f1_en.mul(s1)
...
# # Calculate FLOPs and MACs(PVTv2)   FLOPs: 15.80 G   MACs: 15.77 G   parameters: 114.81 M
