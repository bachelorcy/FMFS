...
from Network.Network_PVTv2 import FMFS  # PVTv2
...
def parser():
    ...
    parser.add_argument('--checkpoint', default='../FMFS-Net/checkpoints/Net_epoch_best(pvt).pth', type=str)  # PVTv2
    # parser.add_argument('--checkpoint', default='../FMFS-Net/checkpoints/Net_epoch_best(fem).pth', type=str)  # FEM Abolation
    # parser.add_argument('--checkpoint', default='../FMFS-Net/checkpoints/Net_epoch_best(frb).pth', type=str)  # FRB Abolation
        ...
        for epoch in range(1, self.args.epoch + 1):
            ...
            if self.use_warmup and epoch <= 5:
                ...
                if epoch >= 1 and cur_score < self.best_score:      # 断点时修改
                    ...
                    # torch.save(self.model.state_dict(), os.path.join(self.args.savepath, 'Net_epoch_best(fem).pth'))  # FEM Abolation
                    # torch.save(self.model.state_dict(), os.path.join(self.args.savepath, 'Net_epoch_best(frb).pth'))  # FRB Abolation
                    # torch.save(self.model.state_dict(), os.path.join(self.args.savepath, 'Net_epoch_best(oct).pth'))  # Oct Abolation
                    torch.save(self.model.state_dict(), os.path.join(self.args.savepath, 'Net_epoch_best(pvt).pth'))  # PVTv2
                    print(f">>> Save state_dict successfully! Best epoch is {epoch}.")

            if epoch >= 1:  # 断点时修改
                # 每10个epoch保存一次权重
                if epoch % 10 == 0:
                    # save_name = f'Net_epoch_{epoch}(fem).pth'  # FEM Abolation
                    # save_name = f'Net_epoch_{epoch}(frb).pth'  # FRB Abolation
                    # save_name = f'Net_epoch_{epoch}(oct).pth'  # Oct Abolation
                    save_name = f'Net_epoch_{epoch}(pvt).pth'  # PVTv2
